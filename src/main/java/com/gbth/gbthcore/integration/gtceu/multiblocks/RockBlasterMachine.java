...
public class RockBlasterMachine extends PrimitiveWorkableMachine implements IUIMachine {
    public RockBlasterMachine(IMachineBlockEntity holder, Object... args) {
        super(holder, args);
    }
    ...
    public ModularUI createUI(Player player) {
        return new ModularUI(176, 166, this, player)
                ...
                .widget(new SlotWidget(exportItems.storage, 1, 123, 38, true, false)
                        .setBackgroundTexture(new GuiTextureGroup(GuiTextures.PRIMITIVE_SLOT, GuiTextures.PRIMITIVE_INGOT_OVERLAY)))
    ...
    public void clientTick() {
        ...
        if(this.recipeLogic.isWorking()) {
            ...
            float xPos = (float) dir.getStepX() * 1.76f + (float) pos.getX() + 0.5f;
            float yPos = (float) dir.getStepY() * 0.76f + (float) pos.getY() + 0.25f;
            float zPos = (float) dir.getStepZ() * 1.76f + (float) pos.getZ() + 0.5f;
